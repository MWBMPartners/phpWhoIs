...
<body>
    ...
    <?php
        // Load build info (generated by deploy action)
        $buildInfo = [];
        if (file_exists(__DIR__ . DIRECTORY_SEPARATOR . 'build_info.php')) {
            require_once __DIR__ . DIRECTORY_SEPARATOR . 'build_info.php';
        }
    ?>
    <!-- Footer -->
    <div class="footer">
<?php
    if (isset($app["Application"]["Version"]["Version"]) && $app["Application"]["Version"]["Version"]) {
        ...
        if (!empty($app["Application"]["Version"]["Development"]["Status"])) {
            echo "&nbsp;".$app["Application"]["Version"]["Development"]["Status"];
        }

        if (!empty($buildInfo['commit_short'])) {
            echo '&nbsp;(<a href="' . htmlspecialchars($buildInfo['commit_url']) . '" target="_blank" class="footer-commit">';
            echo htmlspecialchars($buildInfo['commit_short']);
            echo '</a>';

            if (!empty($buildInfo['commit_date'])) {
                echo '&nbsp;' . htmlspecialchars($buildInfo['commit_date']) . ')';
            }
            else{
                echo ')';
            }
